					...
					if(summaryBar)
					{
						if(summaryBar.strFunction == SummaryBarConstant.FUNC_CUSTOM)
						{
							...
							if(tmpData != null)
							{
								if(column.formatType != null && column.formatType[summaryBar.summaryBarKey] != null)
									originalText = listOwner.summaryBar.formatBaseOnPatternTotal(column,Number(tmpData),data);
								else
						...
						else
						{
							if(isNaN(Number(data[summaryBar.totalColDataField]))) // using for case of user set text is not number
								...
								originalText = listOwner.summaryBar.formatBaseOnPatternTotal(column,data[summaryBar.totalColDataField],data);
						}
					}
